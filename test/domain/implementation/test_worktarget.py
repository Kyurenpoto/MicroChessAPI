...
from domain.implementation.worktarget import ValidWorkTarget, WorkTarget


def test_cannot_castle() -> None:
    with pytest.raises(RuntimeError) as exinfo:
        ValidWorkTarget(WorkTarget(0, [MICRO_STARTING_FEN], [MICRO_CASTLING_SAN])).value()
...
def test_empty_from_square() -> None:
    with pytest.raises(RuntimeError) as exinfo:
        ValidWorkTarget(WorkTarget(0, [MICRO_ONLY_KING_FEN], [MICRO_FIRST_MOVE_SAN])).value()
...
def test_opposite_from_square() -> None:
    with pytest.raises(RuntimeError) as exinfo:
        ValidWorkTarget(WorkTarget(0, [MICRO_STARTING_FEN], [MICRO_SECOND_MOVE_SAN])).value()
...
def test_full_to_square() -> None:
    with pytest.raises(RuntimeError) as exinfo:
        ValidWorkTarget(WorkTarget(0, [MICRO_STARTING_FEN], [MICRO_KING_SIDE_MOVE_SAN])).value()
...
def test_invalid_piece_move() -> None:
    with pytest.raises(RuntimeError) as exinfo:
        ValidWorkTarget(WorkTarget(0, [MICRO_SWAP_KING_BISHOP_FEN], [MICRO_KING_SIDE_MOVE_SAN])).value()
